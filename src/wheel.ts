...
export default class WheelScroller {
  ...
  get directionX() {
    const direction = this.options.direction || (this.scroller.options.scrollX ? 'x' : 'y')
    return direction === 'x'
  }
  ...
  private normalizeOptions() {
    const options = (this.options = isPlainObject(this.options) ? this.options : {})
    if (!options.rotate) {
      options.rotate = 25
    }
  ...
  findNearestValidWheel(point: TranslaterPoint): { index: number; point: TranslaterPoint } {
    ...
    const newPoint = this.directionX ? { x: newOffset, y } : { x, y: newOffset }
  ...
  private tapIntoHooks() {
    ...
    // })
    scroller.on(scroller.eventTypes.translate, (endPoint: TranslaterPoint) => {
    ...
    scrollBehaviorY.hooks.on(
      ...
      (
        ...
        momentumInfo.rate = 4
        if (directionX) {
          momentumInfo.destination = this.findNearestValidWheel({ x: momentumInfo.destination, y: 0 }).point.x
        } else {
          momentumInfo.destination = this.findNearestValidWheel({ x: 0, y: momentumInfo.destination }).point.y
        }
        const maxDistance = 1000
        const minDuration = 800
        if (distance < maxDistance) {
          momentumInfo.duration = Math.max(minDuration, (distance / maxDistance) * scroller.options.swipeTime)
        }
      }
    )
